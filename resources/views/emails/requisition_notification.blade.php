...
<body>
    <div class="container">
        ...
        <table>
            ...
            <tr>
                <th>Program:</th>
                <td>{{ $requisition->program->name ?? 'N/A' }}</td>
            </tr>
            <tr>
                <th>Amount:</th>
                <td>UGX {{ number_format($requisition->amount ?? 0) }}</td>
            </tr>
            <tr>
                <th>Date Submitted:</th>
                <td>{{ \Carbon\Carbon::parse($requisition->created_at)->format('d M, Y') }}</td>
            </tr>
            @if($action == 'rejected' && !empty($requisition->rejection_reason))
            <tr>
        ...
        <p>You can view the requisition details in the system.</p>

        <a href="{{ url('/requisitions/' . $requisition->id) }}" class="button">View Requisition</a>
